 ...
 * 人脸检测区域View
 */
public class FaceDetectRoundViewLand extends View {

    private static final String TAG = FaceDetectRoundViewLand.class.getSimpleName();

    public static final float SURFACE_HEIGHT = 1000f;
    public static final float SURFACE_RATIO = 0.75f;
    public static final float WIDTH_SPACE_RATIO = 0.6f;
    ...
    public FaceDetectRoundViewLand(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        mBGPaint.setColor(COLOR_ROUND);
        mBGPaint.setStyle(Paint.Style.FILL);
        mBGPaint.setAntiAlias(true);
        mBGPaint.setDither(true);

        mPathPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPathPaint.setColor(COLOR_BG);
        ...
        mFaceRoundPaint.setColor(COLOR_BG);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
	    LogUtil.e(TAG,"changed : "+changed+"  left : "+left+"  top : "+top+"  right : "+right+"  bottom : "+bottom);
        float canvasWidth = right - left;
        float canvasHeight = bottom - top;

        float x = canvasWidth / 2;  //960
        //float y = (canvasHeight / 3) - ((canvasHeight /2) * HEIGHT_RATIO); //864
	    //float y = (canvasHeight / 2);   //960
	    float y = canvasHeight / 2 + canvasHeight * HEIGHT_RATIO / 2 + 40;
        float r = (canvasWidth / 2) - ((canvasWidth / 2) * WIDTH_SPACE_RATIO);  //663
	    LogUtil.e(TAG,"x : "+x+"  y : "+y+"  r : "+r);
	    LogUtil.e(TAG,"left : " + (x-r) + " , " + (y-r) + "  right : " + (x+r) + " , " + (y-r));
        if (mFaceRect == null) {
